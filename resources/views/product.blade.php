          ...
          <div class="swiper-wrapper" itemscope itemtype="http://schema.org/ItemList">@foreach ($similar_products as $key => $item)
            <div class="swiper-slide" style="padding: 0 5px 5px 0">
              <div class="item item--for-slider" itemscope itemprop="itemListElement" itemtype="http://schema.org/Product">
                ...
                <div class="item__image"><a href="/catalog/{{$item->category_slug}}/{{$item->slug}}" itemprop="url"></a>
                  <picture>
                    <source srcset="{{$item->image_path[0]['name']}}-small.webp" data-src="{{$item->image_path[0]['name']}}-small.webp" type="image/webp"><img src="{{$item->image_path[0]['name']}}-small.{{$item->image_path[0]['extension']}}" data-src="{{$item->image_path[0]['name']}}-small.{{$item->image_path[0]['extension']}}" alt="{{$item->name}}" title="{{$item->name}}" itemprop="image"><meta itemprop="description" content="{{$item->name}}">
                  </picture>
                </div>
